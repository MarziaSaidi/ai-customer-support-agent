...
public class AnalyticsService {
    ...
    public AnalyticsResponse getCompanyAnalytics(Long companyId) {
        long totalConversations = chatSessionRepository.findByCompanyIdOrderByCreatedAtDesc(companyId).size();
        ...
        double customerSatisfaction = feedbackRepository.findAll().stream()
                .filter(f -> f.getSession().getCompany().getId().equals(companyId))
